...
emotion_labels = ['Angry', 'Disgust', 'Fearful', 'Happy','Neutral', 'Sad', 'Surprise']
...
def extract_features(data, sample_rate):
    ...
    # MFCC
    mfcc = np.mean(librosa.feature.mfcc(y=data, n_mfcc=40, sr=sample_rate).T, axis=0)
...
def home_page(predictions_placeholder):
    ...
    with st.form("my_form"):
        uploaded_file = st.file_uploader("Choose an audio file", type=["wav", "mp3"],key="up")
        ...
        if submitted:
            if uploaded_file is not None:
                ...
                predicted_emotion = np.array(emotion_labels)[predictions[0].astype(int)]
